...
| **Phase 4** Deal Workspace rebuild | ✅ shipped | 2026-04-25 | Second room migrated. Six waves on a single branch + single PR (#8): scaffold + state, read-path persistence, modal interactions + write path, realtime + legacy mirror, filter chips + workspace-health snapshot, legacy flag-redirect. Behind Posthog flag `room_deal_workspace_v2`. |
| **Phase 5+** | ⏳ pending | — | Next room migration per ADR-001 §6 priority list (likely Dashboard, Future Autopsy, or Signal Console). |
