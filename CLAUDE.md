...
- **Readiness Score** — full dark navy (legacy, not migrated)
- **Founding GTM / Handoff Kit** — full dark navy (legacy, not migrated)

These are dropped from the migration list pending a founder-led mind-rethink (per the 2026-04-27 ADR-002 amendment). They retain their dark surface until they're rebuilt; that rebuild will land them on the bright doctrine.
...
- **Readiness Score** — still on legacy, dark (out of migration scope per ADR-002)
- **Founding GTM / Handoff Kit** — still on legacy, dark (out of migration scope per ADR-002)
...
| **Phase 4** overall close | ✅ closed | 2026-04-28 | All 17 in-scope rooms migrated to the new stack + PoC bright flip + doctrine alignment. Readiness Score and Founding GTM stay on legacy per ADR-002 (founder mind-rethink pending). The Phase 4 deletion sweep (legacy `/app/<room>/` + `js/<room>-*.js` for the 17 migrated rooms) is **not done yet**: the legacy paths are still in place because their flag-redirect scripts are the cutover mechanism — they should only be deleted after confirming all founders/operators are on the new rooms with no flag-disabled holdouts. Open question for next session: when to start the deletion sweep. |
