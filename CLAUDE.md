...
|---|---|---|
| 2026-06-01 | App operating timezone → Central (America/Chicago) | **Founder decision: the app operates on Central time, not UTC.** Surfaced during Phase E verification — the schedule picker interpreted "9am" as 9am UTC (4am Chicago), so scheduled skills fired 5-6 hours off. Locked: America/Chicago is the app's operating timezone wherever a wall-clock time matters to the operator. New `src/lib/time/chicago.ts` is the single source of truth for Chicago wall-clock ↔ UTC conversion — Intl-based + DST-aware (CST/UTC-6 winter, CDT/UTC-5 summer), no hardcoded offset, no new dependency. Phase E's `nextFireAt` rewritten to interpret cadence times as Chicago wall-clock (floating-date calendar math, single tz conversion at the boundary); the heartbeat's Deno duplicate carries the synced logic; `scheduled_skills.timezone` stores `America/Chicago`; the schedule modal label is now "Time (Central · CT)". ADR-012 §2 carries the amendment. **Scope note:** this PR fixes the functional case (scheduling fires at Chicago-local time). Cosmetic date-display localization across the other rooms (observation timestamps, "stalled N days") is a separate follow-up sweep, not done here. Test delta: +9 chicago helper tests; scheduling tests rewritten to Chicago semantics (still 17). |
