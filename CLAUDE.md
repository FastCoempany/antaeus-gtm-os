...
- **Phase B supersession:** ADR-004 §"Phasing" defined Phase B as "register the signal-decay generator + ship Dashboard 'this week's reads' card." That scope retires per ADR-006. The Briefing absorbs Phase B entirely. The signal-decay concept becomes one special case of the broader Trigger grammar (silence + threshold triggers against signal recency). The Patterns produced by the Recipe Layer pipeline ARE the observations Phase A's `observations` table was scaffolded for — at much higher quality with audit envelopes attached.
...
3. **Heartbeat** (`supabase/functions/heartbeat/`) — Supabase Edge Function scheduled by pg_cron every 30 minutes. Wakes up, enumerates active workspaces (any session activity or observation in the last 7 days), runs the registered observation generators, writes candidates to the ledger via the dedupe-and-supersession writer. Phase A skeleton has an empty generator registry; Phase B+ registers the first one.
...
- **Phase B (first observable signal)** — register signal-decay generator, ship the Dashboard "this week's reads" card. The first visible payoff.
- **Phase C (skills layer)** — markdown skill recipes, executor, Ctrl+K picker, 3-5 starter skills.
- **Phase D (birdseye strip + inter-room push)** — strip mounts on every room, wired to session + ledger.
- **Phase E (operator scheduling)** — cron-style "remind me Friday", auto-running skills.
...
| **Checkpoint 2** Tier 1 retrofit | ⏳ pending | — | Signal Console → Deal Workspace → Outbound Studio, 5 steps each (audit → schema → dual-write → flip-read → drop-legacy). ~15 PRs total. When all three hit Step 5, Phase B unblocks. |
