...
| **Phase 4 / Rooms 3–19** | ⏳ pending | — | Remaining room migrations in priority order per ADR-001 §6 — Signal Console, Future Autopsy, PoC Framework, Outbound Studio, Cold Call Studio, LinkedIn Playbook, Advisor Deploy, Call Planner, Territory Architect, Sourcing Workbench, ICP Studio, Readiness Score, Quota Workback, Founding GTM, Welcome, Onboarding, Settings. Phase 4 overall gate: all 19 rooms stable + legacy `/app/<room>/` + `js/<room>-*.js` deleted. |
