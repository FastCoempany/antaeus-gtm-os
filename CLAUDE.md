...
- **pg_cron schedule** — `20260519180002_heartbeat_schedule.sql` enables `pg_cron` + `pg_net` extensions and includes the `cron.schedule('antaeus-heartbeat', '*/30 * * * *', ...)` call COMMENTED OUT. Founder uncomments + applies after deploying the function and setting `app.heartbeat_url` + `app.service_role_key` DB settings.
...
- Next: Phase B — register the first generator (signal-decay detection) + ship the visible Dashboard "this week's reads" card. The first observable payoff.
