...
| **Phase 4 / Rooms 11–19** | ⏳ pending | — | Remaining room migrations in priority order per ADR-001 §6, **with two scope changes landed 2026-04-27** — Readiness Score and Founding GTM are dropped from the migration list pending a founder-led mind-rethink on the unique value each surfaces; both stay on legacy until that rethink completes. Onboarding stays in the list but is a greenfield rebuild, not a migration. The 7 rooms still owed in this phase: ICP Studio, Territory Architect, Sourcing Workbench, Quota Workback, Settings, Welcome, Onboarding (greenfield). Phase 4 overall gate (revised): 7 + 10 = 17 rooms stable + legacy `/app/<room>/` + `js/<room>-*.js` deleted for those 17. Readiness Score + Founding GTM remain on legacy and are excluded from the deletion sweep until their mind is redefined. |
