/**
 * Briefing Compose — pure logic (B.9a).
 *
 * Stage 3.8 per Recipe Layer Spec. Runs once per pipeline run after
 * synthesis + contrarian, reads the surfaced Patterns + trigger fires,
 * and produces a single one-or-two-sentence "lead" that opens the
 * briefing — the line a peer would say first if they were handing
 * the operator this week's read out loud.
 *
 * One Sonnet 4.6 call per run, ~250 output tokens. Cheap (~$0.005).
 * Refusing is a valid outcome — when there's nothing to lead with
 * (zero patterns, zero fires), the stage outputs refused=true and no
 * lead lands on the run. The UI renders nothing in that case.
 *
 * Voice per canon Part III §11: plain sentence, no startup jargon, no
 * single-noun abstractions doing the work of a sentence, no "this
 * week" throat-clearing. Two sentences max — the lead is meant to be
...
export interface ComposePatternSummary {
    /** Standard or contrarian — affects how the LLM frames the lead. */
    readonly pattern_type: "standard" | "contrarian";
    readonly title: string;
    /** First sentence of the analysis paragraph (clipped at ~200 chars). */
    readonly summary: string;
    readonly confidence: number;
}

export interface ComposeTriggerFireSummary {
    /** What the operator armed. */
    readonly trigger_natural_language: string;
    /** What the system observed that satisfied it. */
...
export interface ComposeDraft {
    ...
    /** The lead itself. Null when refused=true. */
...
/** Pull a JSON object out of an LLM response. Tolerates code fences. */
...
// Title-cased single-noun jargon that does the work of a sentence — the
// shape canon Part III §11 specifically bans.
...
export function runComposeGate(draft: ComposeDraft): ComposeGateResult {
    // Refusal passes — the absence of a lead is a valid outcome.
    if (draft.refused) {
