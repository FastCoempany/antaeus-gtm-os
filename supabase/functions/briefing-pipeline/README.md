...
Source registry: **populated by B.1b + B.1c, extended in B.2c**. Seven fetchers active — HN Algolia, TechCrunch RSS, PR Newswire personnel, Wikipedia pageviews, GitHub releases atom, HTML diff, and **Signal Console** (the substrate read: reads the signals attached to competitor-flagged accounts and emits them as evidence per canon §4.21). The Signal Console source is the reliable category-specific feed — the operator's curated competitive intelligence — where the external sources are sparse or noisy for a B2B-vendor watchlist. Each signal carries its editorial outlet as a `sc:<outlet>` source_id so two outlets reporting one event count as two distinct sources in the cluster evidence gates, and the `sc:` prefix routes to a high-confidence source config (low volume, high reliability).

Context hydration (ADR-007): the server-side `hydrateContext` reads the commercial-identity layer from Supabase — `workspace_profile` (what we sell), `icps` (target industries + buyers), and `signal_console_accounts` flagged `relationship_type='competitor'` (the competitive set). It derives `watchlist_companies` (competitors + ICP industries) + the ICP body + the commercial profile, which is what the watchlist-driven fetchers (HN Algolia, Wikipedia, GitHub) query on and what anchors enrichment relevance. A workspace with a set commercial profile + flagged competitors gets category-specific intelligence; an empty workspace still runs (the firehose sources TC + PR Newswire produce regardless), with the watchlist-driven sources contributing zero until the operator sets their profile + flags competitors in ICP Studio + Signal Console. Discovery's objection library is the one piece still unwired server-side — it lives in client-side framework runtime data, not Supabase; a future sync path brings it across (ADR-007 §4).
...
B.2b — Stage 3.4 Cluster. Groups enriched items into candidate clusters across three axes (pain_tag, exec_move, narrative_shift via topic tags), computes weighted evidence per Recipe Layer Spec §3.4 (`SRC_CONF × inverse_volume_factor × historical_snr × recency_factor`), applies the qualification rules, and persists qualifying clusters to `briefing_clusters` with trajectory tracking (rising/stable/declining vs the prior run's cluster of the same type+anchor). No LLM cost — pure deterministic math. The relevance gate is waived for uninitialized workspaces (relevance was scored conservatively at enrich time with no watchlist); the evidence + distinct-source gates stay intact. On single-source-heavy data, most narrative clusters won't qualify (distinct_sources ≥ 2 gate); exec_move clusters qualify on a single high-confidence source. Every candidate's evaluation (qualifying + rejected, with reason) is recorded in `stage_log.data.perCluster`.

B.2c — Stage 3.5 Synthesize. For every qualified cluster from Stage 3.4, the multi-stage synthesis runs: **Draft** (Opus 4.7 + extended thinking) → **Critique** (Sonnet 4.6, cross-model ensemble) → **Revise** (Opus 4.7, only when the critique flags issues) → **Quality Gate** (deterministic checks per Voice Document v0.1 §6/§7 — banned vocabulary, analysis 60–240 words, name ≤12 words + declarative, ≤3 hedging adverbs, banned hedge constructions, marketing-soup detection, 1–3 routed recommended moves, six-question slots populated, evidence-id validity, confidence in range). A Pattern persists to `briefing_patterns` (pattern_type='standard') only when it passes the gate; gated-out + errored clusters are recorded in `stage_log.data.perCluster` with the failure reasons. Per-cluster synthesis cost (draft + critique + revise) rolls up to `briefing_runs.total_cost`. Typical cost: $0.08–$0.12 per qualifying cluster. Uses the same `ANTHROPIC_API_KEY` as enrich. The quality gate is the canonical reference at `src/briefing/lib/synthesis/` (vitest-tested); the Edge Function mirrors it verbatim in `llm/synthesis-shared.ts`. Pattern rendering in the briefing room lands in B.2c-2.
