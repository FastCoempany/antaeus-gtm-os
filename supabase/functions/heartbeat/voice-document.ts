/**
 * Voice Document — structured form (Deno-side duplicate).
 *
 * Sync source: src/lib/voice/voice-document.ts.
 * When you change one, change the other. The Deno runtime can't import
 * from the Node-flavored src/ tree, so this file exists to give the
 * Edge-Function generators the same validator the Briefing synthesis
 * side will use.
 *
 ...
 * `supabase/functions/heartbeat/voice/voice-document.ts` so the
 * Edge-Function generators can run the same validator without
 * cross-runtime imports. Both files MUST stay in sync. Each file
 * carries a sync-note pointing at the other.
 */
