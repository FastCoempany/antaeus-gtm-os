...
function buildTerritoryArchitectSeed(mode){
  if(mode==='ent'){
    return {
      ...
      approaches:[
        {id:'ta_ent_ap_1',focusId:'ta_ent_th_1',category:'exec-bridge',name:'CCO integration review',description:'Use the acquisition integration agenda to get the CCO and finance into one room.',status:'active',createdAt:d(108),retiredAt:null},
        {id:'ta_ent_ap_2',focusId:'ta_ent_th_1',category:'ops-proof',name:'Contact-center fracture proof',description:'Lead with tooling fragmentation and queue/reporting inconsistency across sites.',status:'active',createdAt:d(107),retiredAt:null},
        {id:'ta_ent_ap_3',focusId:'ta_ent_th_2',category:'risk-case',name:'Audit deadline opener',description:'Open on the deadline, the communication gap, and the revenue risk if it stays manual.',status:'active',createdAt:d(106),retiredAt:null},
        {id:'ta_ent_ap_4',focusId:'ta_ent_th_2',category:'patient-impact',name:'Outcome protection path',description:'Use patient/service outcomes and reimbursement exposure to force the urgency.',status:'active',createdAt:d(105),retiredAt:null},
        {id:'ta_ent_ap_5',focusId:'ta_ent_th_3',category:'selection-frame',name:'Evaluation criteria reset',description:'Reset the buying criteria around operator proof, not generic AI ambition.',status:'active',createdAt:d(104),retiredAt:null},
        {id:'ta_ent_ap_6',focusId:'ta_ent_th_3',category:'eb-access',name:'Budget-owner bridge',description:'Get to the economic owner before the RFP gets trapped in platform feature scoring.',status:'active',createdAt:d(103),retiredAt:null}
      ],
      accounts:[
        {id:'ta_ent_acct_1',name:'Meridian Logistics Global',tier:1,focusId:'ta_ent_th_1',leverageType:'existing-proof-point',additionTrigger:'signal-event',status:'active',addedAt:d(58),overrideCount:0},
        {id:'ta_ent_acct_2',name:'Cascadia Health Network',tier:1,focusId:'ta_ent_th_2',leverageType:'network-connection',additionTrigger:'signal-event',status:'active',addedAt:d(47),overrideCount:0},
        {id:'ta_ent_acct_3',name:'Trident Pharmaceuticals',tier:1,focusId:'ta_ent_th_2',leverageType:'market-signal',additionTrigger:'signal-event',status:'active',addedAt:d(44),overrideCount:0},
        {id:'ta_ent_acct_4',name:'Northstar Capital Partners',tier:2,focusId:'ta_ent_th_3',leverageType:'market-signal',additionTrigger:'signal-event',status:'active',addedAt:d(38),overrideCount:0},
        {id:'ta_ent_acct_5',name:'Summit Hospitality Group',tier:2,focusId:'ta_ent_th_1',leverageType:'network-connection',additionTrigger:'network-opportunity',status:'active',addedAt:d(33),overrideCount:0},
        {id:'ta_ent_acct_6',name:'Atlas Energy Corp',tier:3,focusId:'ta_ent_th_2',leverageType:'market-signal',additionTrigger:'strategic-bet',status:'active',addedAt:d(61),overrideCount:0},
        {id:'ta_ent_acct_7',name:'Pinnacle Defense Systems',tier:3,focusId:'ta_ent_th_3',leverageType:'cold',additionTrigger:'strategic-bet',status:'active',addedAt:d(67),overrideCount:0}
      ],
      signals:[
  ...
  return {
    ...
    approaches:[
      {id:'ta_mm_ap_1',focusId:'ta_mm_th_1',category:'operator-proof',name:'Volume breakage opener',description:'Lead with the operator pressure from expansion before talking category.',status:'active',createdAt:d(78),retiredAt:null},
      {id:'ta_mm_ap_2',focusId:'ta_mm_th_1',category:'exec-bridge',name:'Owner-plus-backlog path',description:'Get the real owner and backlog consequence into the same conversation.',status:'active',createdAt:d(77),retiredAt:null},
      {id:'ta_mm_ap_3',focusId:'ta_mm_th_2',category:'deadline-frame',name:'Audit deadline route',description:'Open on the date, the workflow gap, and what breaks if they wait.',status:'active',createdAt:d(76),retiredAt:null},
      {id:'ta_mm_ap_4',focusId:'ta_mm_th_2',category:'risk-proof',name:'Revenue/compliance proof',description:'Tie communication failure to measurable compliance or reimbursement risk.',status:'active',createdAt:d(75),retiredAt:null},
      {id:'ta_mm_ap_5',focusId:'ta_mm_th_3',category:'buying-criteria',name:'Operator criteria reset',description:'Reset the evaluation around operator proof instead of AI aspiration.',status:'active',createdAt:d(74),retiredAt:null},
      {id:'ta_mm_ap_6',focusId:'ta_mm_th_3',category:'eb-bridge',name:'Budget-owner bridge',description:'Force access to the budget owner before the evaluation turns generic.',status:'active',createdAt:d(73),retiredAt:null}
    ],
    accounts:[
      {id:'ta_mm_acct_1',name:'Meridian Logistics',tier:1,focusId:'ta_mm_th_1',leverageType:'existing-proof-point',additionTrigger:'signal-event',status:'active',addedAt:d(37),overrideCount:0},
      {id:'ta_mm_acct_2',name:'Cascadia Health Systems',tier:1,focusId:'ta_mm_th_2',leverageType:'network-connection',additionTrigger:'signal-event',status:'active',addedAt:d(28),overrideCount:0},
      {id:'ta_mm_acct_3',name:'Northstar Financial',tier:2,focusId:'ta_mm_th_3',leverageType:'market-signal',additionTrigger:'signal-event',status:'active',addedAt:d(34),overrideCount:0},
      {id:'ta_mm_acct_4',name:'Riverview Insurance',tier:2,focusId:'ta_mm_th_3',leverageType:'existing-proof-point',additionTrigger:'network-opportunity',status:'active',addedAt:d(30),overrideCount:0},
      {id:'ta_mm_acct_5',name:'Beacon Retail Group',tier:2,focusId:'ta_mm_th_1',leverageType:'market-signal',additionTrigger:'strategic-bet',status:'active',addedAt:d(31),overrideCount:0},
      {id:'ta_mm_acct_6',name:'Apex Manufacturing',tier:3,focusId:'ta_mm_th_1',leverageType:'cold',additionTrigger:'signal-event',status:'active',addedAt:d(56),overrideCount:0}
    ],
    signals:[
...
function buildSourcingWorkbenchSeed(mode, territorySeed){
  var focusIds={};
  (territorySeed.focuses||[]).forEach(function(focus){
    focusIds[focus.name]=focus.id;
  });
  var approachIds={};
  (territorySeed.approaches||[]).forEach(function(approach){
    if(!approachIds[approach.focusId]) approachIds[approach.focusId]=[];
    approachIds[approach.focusId].push(approach.id);
  });
  function firstApproach(focusId,offset){
    var list=approachIds[focusId]||[];
    return list[offset||0]||null;
  }

  if(mode==='ent'){
    var entOps=focusIds['Integration pressure before the board notices']||territorySeed.focuses[0].id;
    var entRisk=focusIds['Compliance and outcome exposure are now one problem']||territorySeed.focuses[1].id;
    var entBudget=focusIds['AI budget exists but the owner path is still weak']||territorySeed.focuses[2].id;
    return {
      queryCards:[
        {id:'sw_ent_qc_1',focusId:entOps,platform:'sales-nav',status:'active',createdAt:d(16),updatedAt:d(3),filters:{industry:'Global logistics / contact-center operations',companySize:'5000+ employees',geography:'North America + APAC',behavioralSignal:'Acquisition integration, queue fragmentation, new operator leader',techSignal:'Service Cloud / Genesys / multi-stack',personaTitles:'CCO; VP Customer Operations; Contact Center Director',exclusions:'Single-site teams, pure freight brokers',customNotes:'Target operators inheriting multi-site queue sprawl after acquisition.'}},
        {id:'sw_ent_qc_2',focusId:entRisk,platform:'zoominfo',status:'active',createdAt:d(15),updatedAt:d(4),filters:{industry:'Healthcare systems / regulated service networks',companySize:'2500-25000 employees',geography:'United States',behavioralSignal:'Audit deadline, reimbursement exposure, patient communication gaps',techSignal:'Epic / legacy notification stack / workflow fragmentation',personaTitles:'VP Patient Experience; COO; Compliance Officer',exclusions:'Small clinics, payers-only',customNotes:'Prioritize hard deadlines over generic compliance language.'}},
        {id:'sw_ent_qc_3',focusId:entBudget,platform:'apollo',status:'active',createdAt:d(14),updatedAt:d(2),filters:{industry:'Financial services / enterprise client services',companySize:'1000-10000 employees',geography:'US + EU',behavioralSignal:'RFP, board-approved AI budget, evaluation cycle live',techSignal:'Intercom / Ada / Salesforce eval',personaTitles:'Head of Client Services; VP Digital; COO',exclusions:'Budget talk without active owner',customNotes:'Need both budget gravity and a real operator thread.'}},
        {id:'sw_ent_qc_4',focusId:entOps,platform:'conference',status:'active',createdAt:d(12),updatedAt:d(1),filters:{industry:'Hospitality / multi-property operations',companySize:'3000+ employees',geography:'North America',behavioralSignal:'New digital leader, guest messaging unification, property sprawl',techSignal:'Medallia / guest messaging stack',personaTitles:'CDO; VP Guest Experience',exclusions:'Single-property groups',customNotes:'Use conference attendee lists and follow-on operator hires.'}}
      ],
      prospects:[
        {id:'sw_ent_p_1',name:'HarborSpan Freight',focusId:entOps,sourceType:'query-card',sourceQueryCardId:'sw_ent_qc_1',initialImpression:'Operator reshuffle after APAC integration looks urgent.',stage:'ready',createdAt:d(7),updatedAt:d(1),research:{focusMatch:'strong',entryPoint:'Leah Morgan, VP Customer Operations',suggestedApproach:firstApproach(entOps,0),leverageType:'market-signal',note:'Three queue systems, one new owner, and public hiring pressure in Manila.'}},
        {id:'sw_ent_p_2',name:'Pioneer Health Alliance',focusId:entRisk,sourceType:'query-card',sourceQueryCardId:'sw_ent_qc_2',initialImpression:'Patient communication audit language is explicit.',stage:'ready',createdAt:d(8),updatedAt:d(2),research:{focusMatch:'strong',entryPoint:'Dr. Nia Brooks, VP Patient Experience',suggestedApproach:firstApproach(entRisk,0),leverageType:'network-connection',note:'Audit remediation board deck references call-center handoff failures.'}},
        {id:'sw_ent_p_3',name:'Argent Wealth Services',focusId:entBudget,sourceType:'query-card',sourceQueryCardId:'sw_ent_qc_3',initialImpression:'RFP is live, but operator owner is still fuzzy.',stage:'researched',createdAt:d(9),updatedAt:d(3),research:{focusMatch:'moderate',entryPoint:'Marcus Delaney, Head of Client Experience',suggestedApproach:firstApproach(entBudget,0),leverageType:'market-signal',note:'Budget exists, but committee still framing this as generic AI modernization.'}},
        {id:'sw_ent_p_4',name:'NorthBridge Hospitality',focusId:entOps,sourceType:'query-card',sourceQueryCardId:'sw_ent_qc_4',initialImpression:'Guest messaging stack looks fragmented across brands.',stage:'researched',createdAt:d(6),updatedAt:d(2),research:{focusMatch:'moderate',entryPoint:'Elena Ruiz, SVP Guest Experience',suggestedApproach:firstApproach(entOps,1),leverageType:'network-connection',note:'CDO just hired from Marriott. Strong ownership signal, weaker forcing event.'}},
        {id:'sw_ent_p_5',name:'Solstice Pharma Services',focusId:entRisk,sourceType:'referral',sourceQueryCardId:null,initialImpression:'Advisor intro surfaced reimbursement exposure tied to patient support.',stage:'pushed',createdAt:d(11),updatedAt:d(2),research:{focusMatch:'strong',entryPoint:'Maya Chen, VP Patient Programs',suggestedApproach:firstApproach(entRisk,1),leverageType:'existing-proof-point',note:'Already strong enough to earn territory review and Signal Console timing.'}},
        {id:'sw_ent_p_6',name:'Granite Public Systems',focusId:entBudget,sourceType:'news-article',sourceQueryCardId:null,initialImpression:'AI budget story is loud; operator reality is still thin.',stage:'captured',createdAt:d(4),updatedAt:d(4)},
        {id:'sw_ent_p_7',name:'EverGrid Energy',focusId:entRisk,sourceType:'event',sourceQueryCardId:null,initialImpression:'Regulatory message is real but buyer route is incomplete.',stage:'parked',createdAt:d(10),updatedAt:d(5),research:{focusMatch:'uncertain',entryPoint:'Jonah Patel, Director Customer Programs',suggestedApproach:null,leverageType:'market-signal',note:'Need the true service owner before it deserves a live slot.'}},
        {id:'sw_ent_p_8',name:'Vector Defense Tech',focusId:entBudget,sourceType:'query-card',sourceQueryCardId:'sw_ent_qc_3',initialImpression:'Formal buying process, but no operator pressure.',stage:'rejected',createdAt:d(13),updatedAt:d(6),rejectionReason:'budget without operator pain'}
      ],
      personaMaps:[
        {id:'sw_ent_pm_1',focusId:entOps,title:'VP Customer Operations',alternativeTitles:'Head of Service Delivery; Contact Center VP',role:'decision-maker',priority:'primary-target',typicalConcerns:'Integration chaos, queue visibility, inconsistent operator control across sites.',bestApproach:firstApproach(entOps,0),notes:'Needs a field-control story, not a platform migration pitch.',createdAt:d(13)},
        {id:'sw_ent_pm_2',focusId:entOps,title:'Contact Center Director',alternativeTitles:'Global Service Ops Director',role:'champion',priority:'secondary',typicalConcerns:'Backlog, staffing volatility, reporting gaps after acquisition.',bestApproach:firstApproach(entOps,1),notes:'Often owns the pain but not the budget.',createdAt:d(12)},
        {id:'sw_ent_pm_3',focusId:entRisk,title:'VP Patient Experience',alternativeTitles:'VP Member Experience',role:'decision-maker',priority:'primary-target',typicalConcerns:'Audit deadlines, satisfaction-linked revenue, workflow compliance proof.',bestApproach:firstApproach(entRisk,0),notes:'Responds to consequence and deadline framing.',createdAt:d(13)},
        {id:'sw_ent_pm_4',focusId:entRisk,title:'Chief Compliance Officer',alternativeTitles:'Regulatory Affairs Lead',role:'influencer',priority:'secondary',typicalConcerns:'Documentation, provable controls, deadline credibility.',bestApproach:firstApproach(entRisk,1),notes:'Useful validator but rarely the buying owner.',createdAt:d(11)},
        {id:'sw_ent_pm_5',focusId:entBudget,title:'Head of Client Services',alternativeTitles:'VP Client Operations',role:'champion',priority:'primary-target',typicalConcerns:'Client experience drift, committee sprawl, proving an operator outcome fast.',bestApproach:firstApproach(entBudget,0),notes:'Needs criteria reset before the RFP flattens the story.',createdAt:d(10)},
        {id:'sw_ent_pm_6',focusId:entBudget,title:'COO',alternativeTitles:'Managing Director of Operations',role:'economic-buyer',priority:'situational',typicalConcerns:'Why now, cost of delay, and whether this is a real operator problem.',bestApproach:firstApproach(entBudget,1),notes:'Bring in once operator pain and owner path are explicit.',createdAt:d(9)}
      ]
    };
  }

  var mmOps=focusIds['Ops strain after expansion']||territorySeed.focuses[0].id;
  var mmRisk=focusIds['Compliance force']||territorySeed.focuses[1].id;
  var mmBudget=focusIds['Budget with missing owner']||territorySeed.focuses[2].id;
  return {
    queryCards:[
      {id:'sw_mm_qc_1',focusId:mmOps,platform:'sales-nav',status:'active',createdAt:d(12),updatedAt:d(2),filters:{industry:'Regional logistics / field-service operators',companySize:'750-2500 employees',geography:'North America',behavioralSignal:'Expansion, backlog, new operator leader',techSignal:'Zendesk / Freshdesk / fragmented queue ops',personaTitles:'VP Customer Operations; Support Director',exclusions:'Massive enterprises, consumer-only brands',customNotes:'Hunt for operators opening sites faster than queue control is catching up.'}},
      {id:'sw_mm_qc_2',focusId:mmRisk,platform:'zoominfo',status:'active',createdAt:d(11),updatedAt:d(3),filters:{industry:'Healthcare / regulated service orgs',companySize:'1000-5000 employees',geography:'United States',behavioralSignal:'Audit deadline, reimbursement risk, acquisition integration',techSignal:'Legacy patient messaging / workflow gaps',personaTitles:'Director Patient Experience; Compliance Officer',exclusions:'Small clinics, gov-only orgs',customNotes:'Need a real date or measurable downside, not just “compliance matters.”'}},
      {id:'sw_mm_qc_3',focusId:mmBudget,platform:'apollo',status:'active',createdAt:d(10),updatedAt:d(2),filters:{industry:'Financial services / insurance operations',companySize:'500-3000 employees',geography:'US + EU',behavioralSignal:'AI budget, vendor evaluation, quality slide',techSignal:'Intercom / Ada / Service Cloud',personaTitles:'VP Digital; Head of Claims Ops; Revenue Ops',exclusions:'Pure R&D AI teams',customNotes:'Budget must connect to a visible operator owner and forcing event.'}},
      {id:'sw_mm_qc_4',focusId:mmOps,platform:'conference',status:'active',createdAt:d(9),updatedAt:d(1),filters:{industry:'Retail / DTC operators',companySize:'500-1500 employees',geography:'North America',behavioralSignal:'Channel launch, support surge, staffing strain',techSignal:'Gorgias / support hiring spike',personaTitles:'VP E-Commerce; Customer Experience Lead',exclusions:'Single-location retailers',customNotes:'Use conference attendee lists and operator hiring patterns for capture.'}}
    ],
    prospects:[
      {id:'sw_mm_p_1',name:'BlueHarbor Freight',focusId:mmOps,sourceType:'query-card',sourceQueryCardId:'sw_mm_qc_1',initialImpression:'Queue strain is visible in hiring and review patterns.',stage:'ready',createdAt:d(6),updatedAt:d(1),research:{focusMatch:'strong',entryPoint:'Dana Cole, VP Customer Operations',suggestedApproach:firstApproach(mmOps,0),leverageType:'market-signal',note:'Opened 3 depots in 8 months. Queue complaints are visible in hiring copy and review sites.'}},
      {id:'sw_mm_p_2',name:'HarborCare Clinics',focusId:mmRisk,sourceType:'query-card',sourceQueryCardId:'sw_mm_qc_2',initialImpression:'Audit remediation language is explicit.',stage:'ready',createdAt:d(7),updatedAt:d(2),research:{focusMatch:'strong',entryPoint:'Alicia Monroe, Director Patient Experience',suggestedApproach:firstApproach(mmRisk,0),leverageType:'network-connection',note:'Two new clinics acquired. Deadline is now tied to reimbursement reporting.'}},
      {id:'sw_mm_p_3',name:'PineGate Insurance',focusId:mmBudget,sourceType:'query-card',sourceQueryCardId:'sw_mm_qc_3',initialImpression:'AI budget exists, but owner path still feels soft.',stage:'researched',createdAt:d(8),updatedAt:d(3),research:{focusMatch:'moderate',entryPoint:'Micah Doyle, VP Digital',suggestedApproach:firstApproach(mmBudget,0),leverageType:'market-signal',note:'Budget committee is real. Still need the budget owner and current workflow pain in the same thread.'}},
      {id:'sw_mm_p_4',name:'NorthPeak Commerce',focusId:mmOps,sourceType:'query-card',sourceQueryCardId:'sw_mm_qc_4',initialImpression:'DTC support is scaling faster than the team.',stage:'researched',createdAt:d(5),updatedAt:d(2),research:{focusMatch:'moderate',entryPoint:'Rachel Kim, VP E-Commerce',suggestedApproach:firstApproach(mmOps,1),leverageType:'market-signal',note:'Good operator route. Still need stronger proof that backlog is hurting the business now.'}},
      {id:'sw_mm_p_5',name:'Luma Fleet Services',focusId:mmOps,sourceType:'referral',sourceQueryCardId:null,initialImpression:'Advisor intro surfaced a real backlog and new operator owner.',stage:'pushed',createdAt:d(9),updatedAt:d(2),research:{focusMatch:'strong',entryPoint:'Nora Vasquez, Head of Customer Operations',suggestedApproach:firstApproach(mmOps,0),leverageType:'existing-proof-point',note:'Already clean enough for territory and signal timing.'}},
      {id:'sw_mm_p_6',name:'Cedar Ridge Health',focusId:mmRisk,sourceType:'news-article',sourceQueryCardId:null,initialImpression:'Acquisition signal is visible but the human owner is still unproven.',stage:'captured',createdAt:d(4),updatedAt:d(4)},
      {id:'sw_mm_p_7',name:'Summit Claims Group',focusId:mmBudget,sourceType:'event',sourceQueryCardId:null,initialImpression:'Strong budget story, thin operator route.',stage:'parked',createdAt:d(10),updatedAt:d(5),research:{focusMatch:'uncertain',entryPoint:'Elijah Park, Director Innovation',suggestedApproach:null,leverageType:'market-signal',note:'Feels like budget theater until the operator owner is named.'}},
      {id:'sw_mm_p_8',name:'Atlas Growth Labs',focusId:mmBudget,sourceType:'query-card',sourceQueryCardId:'sw_mm_qc_3',initialImpression:'Wanted AI language, no real pressure.',stage:'rejected',createdAt:d(12),updatedAt:d(6),rejectionReason:'category fit without operator pressure'}
    ],
    personaMaps:[
      {id:'sw_mm_pm_1',focusId:mmOps,title:'VP Customer Operations',alternativeTitles:'Head of Support; VP Service Ops',role:'decision-maker',priority:'primary-target',typicalConcerns:'Backlog, inconsistent quality, new-site pressure, and lack of queue control.',bestApproach:firstApproach(mmOps,0),notes:'Needs proof the rail starts with real operator pain.',createdAt:d(13)},
      {id:'sw_mm_pm_2',focusId:mmOps,title:'Support Director',alternativeTitles:'Customer Experience Director',role:'champion',priority:'secondary',typicalConcerns:'Capacity, staffing, escalation load, and visibility.',bestApproach:firstApproach(mmOps,1),notes:'Often gives the best field proof.',createdAt:d(12)},
      {id:'sw_mm_pm_3',focusId:mmRisk,title:'Director Patient Experience',alternativeTitles:'Director Member Experience',role:'decision-maker',priority:'primary-target',typicalConcerns:'Audit exposure, communication breakdowns, and reimbursement risk.',bestApproach:firstApproach(mmRisk,0),notes:'Responds to date-driven forcing language.',createdAt:d(11)},
      {id:'sw_mm_pm_4',focusId:mmRisk,title:'Compliance Officer',alternativeTitles:'Quality Director',role:'influencer',priority:'secondary',typicalConcerns:'Documentation, provable workflows, and deadline credibility.',bestApproach:firstApproach(mmRisk,1),notes:'Useful validation path, rarely the primary owner.',createdAt:d(10)},
      {id:'sw_mm_pm_5',focusId:mmBudget,title:'VP Digital',alternativeTitles:'Head of Innovation',role:'champion',priority:'primary-target',typicalConcerns:'Vendor evaluation drift, proving ROI, and keeping AI work tied to operations.',bestApproach:firstApproach(mmBudget,0),notes:'Can open the deal but needs the operator owner alongside them.',createdAt:d(9)},
      {id:'sw_mm_pm_6',focusId:mmBudget,title:'COO',alternativeTitles:'Operations Chief',role:'economic-buyer',priority:'situational',typicalConcerns:'Why now, owner clarity, and budget discipline.',bestApproach:firstApproach(mmBudget,1),notes:'Bring in once the operator problem is named, not before.',createdAt:d(8)}
    ]
  };
}
...
window.seed=function(mode){
  ...
  w('gtmos_ta_focuses',territorySeed.focuses);
