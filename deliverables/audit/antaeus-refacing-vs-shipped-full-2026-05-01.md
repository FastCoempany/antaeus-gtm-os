...
1. **Sentence-shaped headline copy is the most universal drift.** Almost every audited room ships with a topbar title that reads as a category ("DASHBOARD", "FOUNDING GTM", "QUOTA WORKBACK") rather than as the authored headline the picked triptych intended ("The system is live enough to stop pretending.", "What your first hire opens on day one.", "Make the math feel daily."). Phase 2's per-room rework should batch this as a shared copy pass — not 20 separate decisions.
...
Total Phase 2 rework estimate: **2 medium-or-larger structural reworks** (Deal Workspace + Future Autopsy) + **2 medium reworks** (PoC Framework + maybe LinkedIn Playbook for the dark-stage flip) + **~14 trim/copy passes** (mostly headline copy authoring + small per-room polish).
...
- **Punch list:**
  1. Spot-check headline copy on each step against "sentence-shaped not categorical" rule — ensure each step's title reads as authored direction, not as a form-field label
...
- **Punch list:**
  1. Rename `icp-hero__statement` content to match variant C's authored headline register
...
- **AI pick:** based on the shipped `ta-block` + `ta-approach-list` + `ta-form` pattern, the picked direction was clearly the **tier-tinted focus cards + 300-cap account grid** approach. Confirmed by canon §4.5 primitive list. Naming this the AI-pick: **"Tier-allocation as resource-bet"** variant.
...
- **Punch list:**
  1. Sentence-shaped headline copy review on `ta-block__title` and `ta-block__kicker` — confirm they read as authored allocation bets, not categorical "Tier 1 / Tier 2" labels
...
- **Verdict:** **match.** Implementation follows canon §4.6 + the strategic logic (push-engine tied to focus, query cards reproducible, 5 stages enforced).
- **Punch list:**
...
- **AI pick:** **variant B — "Weave opener, objection, proof, and ask into one live route."** Rationale: matches canon §4.9's 6-thread spine (prep / opener / pressure / proof / ask / exit) most directly. The "weave" headline frames the room's TalkLoom as integrative rather than fragmenting; A is too dramatic, C too narrow.
...
- **Punch list:**
  1. Topbar/loom-read headline copy — verify it reads as variant B's "weave the route" register, not generic "thread" labels
...
- **Verdict:** **match.** Cue ladder + cinematic stage + ledger maps to variant 2's "enter only when the room gives a cue" headline.
- **Punch list:**
...
- **Picked artifact:** triptych file exists with headline "Three real command grammars for the next room."
...
- **Punch list:**
  1. Sentence-shaped headline copy on the topbar — currently "CALLS FAMILY" kicker per canon line; verify the title reads as a pre-conviction declaration
...
- **Punch list:**
  ...
  3. Sentence-shaped headline: variant-01c is "Forensic light table" framed; verify the topbar reads in that voice

---

## Room: PoC Framework (Decision Bench)

- **Picked artifact:** none recorded with `-selected-`. Three radical-triptych versions exist (v1, v2, v3). v3 headline: "Proof is not a page. It is a forced event."
- **AI pick:** **v3 headline — "Proof is not a page. It is a forced event."** Rationale: latest revision; matches canon §4.15 mind ("raw interest is not proof until it can be carried"). The forced-event framing carries the discipline that pilot evidence has to be act-on-able by the time the readout meeting happens.
...
- **Verdict:** **partial.** The 5-mold grid + heat ledger + 4-tab docs match the canon mind. Drift: the forge → cast structural metaphor (raw → refined) is currently rendered as 2 columns side-by-side; v3's headline suggests a more directional/temporal flow ("forced event" implies sequence, not simultaneity).
- **Punch list:**
...
- **Picked artifact:** none recorded with `-selected-`. Two radical-triptych versions; v2 headline: "Influence is an asset. Spend it precisely."
...
- **Punch list:**
  ...
  2. Sentence-shaped headline: hero copy should declare the workspace's pressure, not just display the number
