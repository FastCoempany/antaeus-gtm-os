...
| EB-R09 | Persistence control state could leak between demo and real workspace flows, and durable docs could fail cloud sync on nullable defaults. | Durable workspace truth stops being believable if demo, no-auth, import, delete, and cache states can bleed into each other, or if import/delete fail because `sequences.data` rejects `null`. Users also need Settings to explain what belongs to cloud truth versus browser-only aids. | Phase 17 hardened [demo-seed.html](c:/AppDev/v1AntaeusApp/Appv2_290126/demo-seed.html), [app/settings/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/settings/index.html), [js/data-manager.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/data-manager.js), and [js/supabase-config.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/supabase-config.js) so demo cleanup is deterministic, demo exit clears real control flags, import/delete normalize persistence state, and nullable durable docs serialize safely into the non-null `public.sequences.data` column. Phase 44 deepens the Settings surface so backup/restore/delete and scope boundaries are legible: account-level versus workspace-level controls, cloud truth versus local fallback, and browser-only demo state are now called out explicitly. Live browser validation is still required before closure. | Trust, Policy & Workspace Ops | Phase 17 / Phase 44 | P0 | local-patch |
...
| ID | Finding | Why It Matters | Proof | Owner | Target Phase | Priority | Status |
|---|---|---|---|---|---|---|---|
| EB-A01 | Welcome corridor is real, but still not yet the complete first-session system. | The welcome layer needs to teach the landscape, route correctly, remain revisitable, and now clearly connect into week-one rhythm. | Phase 22 upgrades [app/welcome/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/welcome/index.html) into a more explicit activation corridor. Phase 43 deepens it with better-ranked first actions, activation-context chips, explicit week-one rhythm, and a stronger return/help state so the page matters on revisit instead of only on first entry. Live browser validation is still required before closure. | Auth, Onboarding & Activation | Phase 22 / Phase 43 | P1 | local-patch |
...
| EB-A05 | Onboarding existed, but it still behaved more like setup than activation. | If onboarding feels like form fill, the product loses momentum before the welcome corridor can take over. | Phase 42 updates [app/onboarding/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/onboarding/index.html) with role-aware copy, live activation previews, stronger finish-state expectations, and a saved `gtmos_activation_context` object so onboarding config becomes reusable system truth. Live browser validation is still required before closure. | Auth, Onboarding & Activation | Phase 42 | P1 | local-patch |
...
| EB-M07 | Playbook / handoff can become disconnected if upstream modules stay sparse. | The supreme output is only as good as the evidence feeding it. | Phase 40 updates [app/founding-gtm/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/founding-gtm/index.html) with top-level handoff readiness, section states that distinguish empty vs partial vs handoff-ready, weak-source warnings, and a more serious export format. Live browser validation is still required before closure. | Planning, Readiness & Handoff | Phase 40 | P1 | local-patch |
| EB-M08 | Readiness is meaningful, but score reasoning must get more legible and actionable. | If the score feels arbitrary, it loses strategic authority. | Phase 41 updates [app/readiness/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/readiness/index.html) with a visible `why this number` layer, strongest/weakest dimension framing, biggest-unlock rows, stronger action cards, and a saved readiness snapshot so the score can act like reusable system truth. Live browser validation is still required before closure. | Planning, Readiness & Handoff | Phase 41 | P2 | local-patch |
