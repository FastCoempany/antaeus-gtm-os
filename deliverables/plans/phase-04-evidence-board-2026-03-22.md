...
| ID | Finding | Why It Matters | Proof | Owner | Target Phase | Priority | Status |
|---|---|---|---|---|---|---|---|
| EB-R01 | Discovery Studio can appear not to load. | This directly breaks one of the most important execution surfaces. | [app/discovery-studio/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/discovery-studio/index.html) ends with swallowed catches like `bootDiscoveryStudio().catch(function(){})`, so runtime failure can render as a blank or inert module. | Discovery, Deal & Proof Execution | Phase 34 | P0 | open |
| EB-R02 | Sidebar scroll position reset instead of respecting where the user clicked. | It breaks continuity in a long nav and makes the shell feel unstable. | Phase 14 added session-based sidebar scroll persistence, last-module context persistence for non-module pages, and active/context restore logic in [js/nav.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/nav.js). Manual browser verification is still recommended before treating it as fully closed. | Product Shell & Navigation | Phase 14 | P1 | local-patch |
| EB-R03 | PoC Framework can show duplicate progress/banner chrome. | Duplicate injected UI makes the product feel buggy and stacked. | User-reported stacked progress banner; [app/poc-framework/index.html](c:/AppDev/v1AntaeusApp/Appv2_290126/app/poc-framework/index.html) loads many shared injectors and calls `saveIndicator.mountAfter('.page-title')`. | Discovery, Deal & Proof Execution | Phase 37 / Phase 15 | P1 | open |
| EB-R04 | Shared chrome injection rules are uneven. | Repeated injection patterns can create duplicate or conflicting UI across modules. | Audit evidence in shared injectors like [js/guided-rail.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/guided-rail.js) and [js/save-indicator.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/save-indicator.js). | Product Shell & Navigation | Phase 15 | P1 | open |
| EB-R05 | Some user-facing failures are still swallowed instead of surfaced. | Silent failure destroys trust and makes debugging impossible for real users. | Audit explicitly flagged silent catches; Discovery Studio is the clearest example, but the issue is broader. | Product Shell & Navigation | Phase 16 | P0 | open |
...
| EB-R07 | Data-flow nav badge logic is likely broken against the current nav implementation. | Surface-level progress signals may not actually appear where intended. | [js/data-flow.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/data-flow.js) looks for `a.sidebar-link[...]`; [js/nav.js](c:/AppDev/v1AntaeusApp/Appv2_290126/js/nav.js) renders `.nav-item`. | Product Shell & Navigation | Phase 15 | P2 | open |
