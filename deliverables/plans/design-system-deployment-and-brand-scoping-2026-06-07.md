...
5. **localStorage substrate fix** (~3–5 days; gates the Dashboard's correctness) — per the localStorage inventory audit (`deliverables/audit/localstorage-inventory-2026-06-08.md`), the Dashboard's command-engine reads four cross-room snapshots, one of which (`gtmos_readiness_snapshot`) has no writer at all. Three orphans (the readiness snapshot, `gtmos_activation_context`, `gtmos_readiness_last_verdict`) need resolution, the demo-seed pipeline needs to either write the four snapshots or guarantee boot ordering of the producing rooms, and the alias `gtmos_deal_room_health` plus the dead `gtmos_handoff_exported` get cleaned up. Without this, the new Dashboard rebuilds the empty-state bug at a prettier surface. Optionally — recommended but not blocking — start the ADR-005 Step-3 dual-writes on Deal Workspace and Quota Workback so the new Dashboard reads cloud-canonical from day one.
