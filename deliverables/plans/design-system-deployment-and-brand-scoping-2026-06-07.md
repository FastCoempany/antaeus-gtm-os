...
2. **Lexicon enforcement build** (validator + banned-vocabulary + blessed-labels + family-temperatures + CI gate), then the `t()` migration room-by-room — the long pole, starts immediately, independent of brand. **First radiation ring DONE 2026-06-12:** Dashboard (all components + readiness anchor/drawer) + Signal Console + Deal Workspace, ~234 strings declared and CI-gated, three banned-vocab rewordings ("earned", "verdict"-as-container, "unlock"), 0 waivers. Engine-side narrative builders (brief-narrative, scoring reasons, gateBlockers) migrate with their rooms' design-system passes. **Ring 2 DONE 2026-06-12** (Readiness engine + Quota Workback + Outbound Studio). **Rings 3–6 DONE same day — the room-by-room migration is COMPLETE for component-level operator strings across all 21 rooms + RoomChrome**: 1,117 declared t() strings product-wide, 0 of 10 waivers used. The gate forced seven rewordings during rings 3–6 ("Agenda spine"→"Call agenda", "Kill switch verdict"→"Kill switch", "Verdict mode"→"Left alone or corrected", "Leverage"→"Entry point", "Pick the move that compounds"→"Pick the action that compounds", "Unlocks"→"Opens up", "What unlocks next"→"What opens up next"). Out of scope by design: deep content engines (Discovery framework runtime data, autopsy narrative tables, Founding GTM authored sections, Briefing LLM output) — each is governed by its own pipeline (the Briefing already validates at synthesis via the shared voice module); they get wrapped or validated with their rooms' design-system passes.
3. **Brand identity pass, internal, in parallel** — logo/wordmark rounds, color confirmation, the visitor register, the marketing visual system; lands as sibling `10`. **Round 1 decided 2026-06-12 (Grounded A); round 2 artifact shipped 2026-06-12** (`deliverables/mockups/brand-identity-round-2-2026-06-12.html`): stroke-weight studies (W1–W3), crossbar + overhang proportions (P1–P3), the favicon ladder with the no-bar-at-16px simplification, the L1 serif / L2 caps lockup pair, and the two first in-situ placements (landing hero corner + Wayfinder home affordance). **Round 2 decided 2026-06-12 (founder delegated to the recommended picks):** weight **W2** (3.2 at display scale — matches the icon keyline ratio so mark and glyph set read as one hand); proportions **P2** (round-1 crossbar height, longer ground overhang `M2 38h44` so the ground reads as ground, not underline); favicon ladder confirmed with the **no-bar-at-16px** simplification (legs + ground only — the ground line is the signature); lockup pairing confirmed — **L1 serif** (DM Serif Display) for landing + docs, **L2 letterspaced caps** for product chrome, reversed L1 for legal/footer only. Round 3 (next): lock exact geometry into sibling spec 10, ship the real favicon files, replace the wordmark in RoomChrome + the landing page.
