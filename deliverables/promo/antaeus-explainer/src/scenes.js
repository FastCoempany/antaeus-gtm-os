...
const HIRE = { seed: 130, flip: -1, jacket: P.C.forest, hair: P.C.hairBrown, hairStyle: 1 };
const FOUNDER = { seed: 100, jacket: P.C.ink, hair: P.C.hairDark };
const tileW = 178, tileH = 72; const TILE = (i) => [1330 + (i % 3) * (tileW + 8), GY - tileH - Math.floor(i / 3) * (tileH + 6)];
const HIRE_WAIT = 1935; // the hire waits at the right margin from the head beat to the book beat
...
export function plan(cues) {
  ...
  const T = {
    ...
    // head
    bubble: B.head.start + 0.15, hireIn: B.head.start + 0.3, hireStop: B.head.start + 1.9, handover: wordAt(B.head, 'hand', 0.62) - 0.15, snap: wordAt(B.head, 'hand', 0.62) + 0.55, qmark: wordAt(B.head, 'hand', 0.62) + 0.95, liftText: wordAt(B.head, 'hand', 0.62), tile: B.head.start + 0.9,
    // myth
    ...
    heave: Math.min(wordAt(B.myth, 'lifted', 0.78) - 0.1, B.myth.end - 3.0), liftedLabel: Math.min(wordAt(B.myth, 'lifted', 0.78) + 0.5, B.myth.end - 2.4), reground: Math.max(Math.min(wordAt(B.myth, 'weak', 0.93) + 0.45, B.myth.end - 1.3), Math.min(wordAt(B.myth, 'lifted', 0.78) - 0.1, B.myth.end - 3.0) + 1.5), giantFade: B.tell.start + 0.1, markIn: B.tell.start + 0.5,
    // tell
    tellWalk: B.tell.start + 0.2, card1: Math.max(wordAt(B.tell, 'who', 0.18) - 0.1, B.tell.start + 1.3), card2: Math.max(wordAt(B.tell, 'chasing', 0.55) - 0.35, B.tell.start + 2.6), fills: wordAt(B.tell, 'fills', 0.86) - 0.1,
    // morning
    sunrise: B.morning.start, relabel: B.morning.start + 0.6, square: B.morning.start + 0.5, plane: wordAt(B.morning, 'morning', 0.12) + 0.3, planeLand: wordAt(B.morning, 'morning', 0.12) + 1.2, stackUp: wordAt(B.morning, 'one', 0.5) - 0.4, oneCard: wordAt(B.morning, 'one', 0.5), why: wordAt(B.morning, 'why', 0.9) - 0.1,
    // slip
...
export function buildFilm(cues) {
  ...
  function desk(ctx, t, b) {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const x = lerp(hx + ox, hx - 20 + (i % 2) * 2, sq), y = lerp(hy + oy, hy - 56 - i * 1.6, sq) - (1 - p) * 180;
    ...
    if (t > T.loud) { const lp = E(t, T.loud, T.loud + 0.5, ease.outBack); const hop = (t < T.square) ? Math.abs(Math.sin((t - T.loud) * 5.1)) * 26 : 0; const x = lerp(hx + 40, hx - 10, sq), y = hy - 190 * (1 - sq) - 140 * sq - hop - (1 - lp) * 200; const rotate = lerp(0.12, 0.03, sq);
      ...
      P.ink(ctx, sq < 0.5 ? [[-26, 12], [-10, 30], [10, 30], [26, 12]] : [[-24, 22], [24, 22]], { width: 4, seed: 398, boil: b, jitter: 0.6, curve: true });
      ctx.restore(); }
    ctx.restore();
  }
  ...
  function stateTile(ctx, t, b) {
    ...
    P.text(ctx, after ? 'HIRE-READY' : 'You are the system', 0, 11, { font: after ? 'sans' : 'hand', size: after ? 30 : 32, weight: 700, align: 'center', color: after ? P.C.forest : P.C.ink70, spacing: after ? 2 : 0, rotate: -0.02 });
    ctx.restore();
  }
  ...
  function world(ctx, t) {
    ...
    const mIn = E(t, T.markIn, T.markIn + 0.5, ease.outBack); if (mIn > 0) { ctx.save(); ctx.translate(1040, GY); ctx.scale(mIn, mIn); P.mark(ctx, 0, 0, 120, { boil: b, groundLen: 0 }); ctx.restore(); }
    ...
    if (t < T.bubble) { const pressing = E(t, T.notes + 0.8, T.notes + 1.4); pose.armL = 1.1 * pressing; face.brows = -0.3 * pressing; face.mouth = 0.25 - 0.5 * pressing; }
      ...
      const w = E(t, T.tellWalk, T.tellWalk + 0.9, ease.inOutCubic); fx = lerp(FX, 880, w); moving = w > 0 && w < 1;
    ...
    } else if (t < B.slip.start) { const w = E(t, T.sunrise, T.sunrise + 0.9, ease.inOutCubic); fx = lerp(880, FX, w); moving = w > 0 && w < 1; face.mouth = 0.5; face.brows = 0.35; }
    else if (t < B.book.start) { // slip: runs over, reaches, misses, picks up the strip, presses the card down
      const run = E(t, T.run, T.run + 0.9, ease.inOutCubic); fx = lerp(FX, 900, run); moving = run > 0 && run < 1;
      const reach = pulse(t, T.miss - 0.3, T.miss + 0.7, 0.3); const pick = pulse(t, T.pickStrip - 0.25, T.pickStrip + 0.5, 0.25); const press = pulse(t, T.press - 0.35, T.press + 0.6, 0.3);
      pose.armR = 0.2 + 2.5 * reach + 0.9 * press - 0.5 * pick; pose.armL = 1.2 * pick; pose.bob = -reach * 14; pose.lean = 0.12 * press; face.brows = -0.5 + 0.9 * E(t, T.press, T.press + 0.5); face.mouth = -0.3 + 0.9 * E(t, T.press, T.press + 0.5);
    } else if (t < B.hire.start) { const w = E(t, T.gather, T.gather + 1.0, ease.inOutCubic); fx = lerp(900, 880, w); moving = w > 0 && w < 1; face.mouth = 0.55; face.brows = 0.35; }
      ...
      fx = lerp(lerp(880, FX, back), 150, lean); moving = (back > 0 && back < 1) || (lean > 0 && lean < 1); flip = lean > 0.15 ? 1 : -1;
    ...
    if (t > T.bubble && t < T.taut) {
      ...
      if (bIn > 0.3 && deflate > 0.05) { const tx = fig.head[0] + 40, ty = fig.head[1] - 20; const r = P.rng(88 + b % 3); const pts = [[tx, ty]]; for (let i = 1; i <= 7; i++) pts.push([lerp(tx, bx - 30, i / 8) + P.rnd(r, -40, 40) * deflate, lerp(ty, by + bh * 0.4, i / 8) + P.rnd(r, -36, 36) * deflate]); pts.push([bx - 30, by + bh * 0.45]); P.ink(ctx, pts, { width: 3, seed: 89, boil: b, jitter: 1.5, curve: true, color: P.C.ink70, alpha: deflate }); const knot = [[tx + 60, ty - 40], [tx + 90, ty - 10], [tx + 50, ty], [tx + 85, ty - 45], [tx + 55, ty - 25], [tx + 95, ty - 28]]; P.ink(ctx, knot, { width: 3, seed: 90, boil: b, jitter: 1.4, curve: true, color: P.C.ink70, alpha: deflate }); const loose = [[tx + 70, ty + 10], [tx + 90, ty + 120], [tx + 60, ty + 260], [tx + 110, GY - 4]]; P.ink(ctx, P.partial(loose, deflate), { width: 3, seed: 91, boil: b, jitter: 1.5, curve: true, color: P.C.ink70, alpha: deflate }); }
      if (bw > 4) { P.cloud(ctx, bx, by, bw, bh, { seed: 400, outline: 4 }); for (let k = 0; k < 2; k++) P.paperBlob(ctx, fig.head[0] + 70 + k * 34 + ho * 60 * k, fig.head[1] - 110 - k * 62, 16 + k * 10, 14 + k * 8, { seed: 410 + k, n: 9, wob: 0.06, outline: 3, alpha: deflate });
    ...
    if (t > T.hireWalk) { const hw = E(t, T.hireWalk, T.hireWalk + 1.2, ease.inOutCubic); const hx = lerp(HIRE_WAIT, 1480, hw); const pick = E(t, T.pickup, T.pickup + 0.7, ease.inOutCubic); const wk = hw < 1 ? walk(t, 2.4) : standing;
      ...
      for (let i = 0; i < 6; i++) { const fp = E(t, T.fan + i * 0.1, T.fan + i * 0.1 + 0.5, ease.outBackSoft); if (fp <= 0) continue; const tx = i < 3 ? 600 + i * 180 : 1650 + (i - 3) * 170; ctx.save(); ctx.globalAlpha = fp; ctx.translate(lerp(hx, tx, fp), GY - tileH * 0.7); ctx.scale(0.7, 0.7); P.dealCard(ctx, 0, 0, tileW, tileH, { name: COMPANIES[i], small: true, seed: 520 + i }); ctx.restore(); } }

    // ── myth: the hand pulls the string taut; the giant; lifted / grounded ──
    if (t > T.handIn && t < T.taut + 0.6) { const hIn = E(t, T.handIn, T.pinch, ease.outCubic); const pullP = E(t, T.pull, T.taut, ease.inOutCubic); const out = E(t, T.taut, T.taut + 0.6, ease.inCubic); const sx = fig.head[0] + 150; const hx = lerp(2100, sx, hIn) + pullP * 1500 + out * 600, hy = lerp(GY - 300, GY - 10, hIn) - (1 - hIn) * 0 + out * 300; P.hand(ctx, hx + 0, hy, 150, Math.PI, { seed: 200, pointing: false }); }
    const gIn = bounceIn(t, T.giantIn, 0.7);
    if (gIn > 0 && t < T.giantFade + 0.75) { const heave = E(t, T.heave, T.heave + 0.9, ease.inOutCubic) * (1 - E(t, T.reground - 0.35, T.reground, ease.inCubic)); const fade = E(t, T.giantFade, T.giantFade + 0.5, ease.inCubic); const gx = 1040;
      ...
      doodle(ctx, 'lifted = weak · same with deals', 1330, 380, T.liftedLabel, t, { size: 44, rotate: 0.04, color: P.C.ink70, alpha: 1 - E(t, T.reground, T.reground + 0.4) });
      doodle(ctx, 'grounded = strong', 1330, 820, T.reground + 0.35, t, { size: 42, rotate: -0.02, color: P.C.forest, alpha: 1 - E(t, T.giantFade, T.giantFade + 0.4) }); }
    if (t > T.title && t < B.tell.start + 0.4) { const a = 1 - E(t, B.tell.start, B.tell.start + 0.4); serifLine(ctx, 'Antaeus', 640, 200, T.title, t, { size: 110, alpha: a }); doodle(ctx, 'named for the giant', 640, 262, T.named, t, { size: 40, align: 'center', rotate: -0.02, color: P.C.ink70, alpha: a }); }
    ...
    if (t > T.card1 - 0.3 && t < T.gather + 1.2) {
      ...
      if (c1 > 0 && tellFade > 0) { const cw = 220, ch = 210, x = 1090, y = GY - ch - (1 - c1) * 380; ctx.save(); ctx.globalAlpha = tellFade; P.paperRect(ctx, x, y, cw, ch, { fill: P.C.cream, seed: 700, shadow: 0.18, dy: 5, outline: 4, rotate: -0.02, cx: x + cw / 2, cy: y + ch / 2 }); P.paperBlob(ctx, x + cw / 2, y + 76, 36, 40, { fill: P.C.skin, seed: 701, n: 10, wob: 0.05 }); P.paperShape(ctx, P.rough([[x + 66, y + 160], [x + 176, y + 160], [x + 160, y + 112], [x + 82, y + 112]], 702, 1.5), { fill: P.C.blue, shadow: 0.1, curve: true }); P.text(ctx, 'who we sell to', x + cw / 2, y + 194, { font: 'hand', size: 30, weight: 700, align: 'center' }); ctx.restore(); }
      ...
      const FILLS = [['Boeing · new head of operations', 0], ['Notion · hiring twelve sellers', 5], ['Chipotle · opening forty stores', 3]];
      FILLS.forEach(([str, k], i) => { const at = T.fills + i * 0.45; const p = E(t, at, at + 0.6, ease.outBackSoft); if (p <= 0) return; const gth = E(t, T.gather, T.gather + 0.8, ease.inOutCubic); const x0 = 1330, y0 = GY - 2 * tileH - 70 - i * 60; const x = lerp(lerp(x0 + 60, x0, p), 1250 - 120, gth), y = lerp(y0, GY - 140, gth); if (p > 0.9 && gth <= 0 && t < T.oneCard + 0.6) { const [ax, ay] = TILE(k); P.doodleArrow(ctx, [x0 + 300 + i * 10, y0 + 22], [ax + tileW / 2 + 40, ay + 4], { boil: b, curve: 0.18, width: 3, color: P.C.blue, head: 12, p: E(t, at + 0.5, at + 0.9) }); } const a = (1 - E(t, T.oneCard + 0.6, T.oneCard + 1.2)); ctx.save(); ctx.globalAlpha = a; ctx.translate(x, y); ctx.scale(lerp(1, 0.7, gth), lerp(1, 0.7, gth)); P.paperLabel(ctx, str, 0, 0, { font: 'hand', size: 28, weight: 700, fill: P.C.bluePaper, color: P.C.ink, seed: 730 + i, rotate: (i - 1) * 0.02, padY: 3, pad: 12, alpha: p }); ctx.restore(); });
      if (tellFade > 0) serifLine(ctx, 'Tell it once.', W / 2, 200, T.card1 + 0.2, t, { size: 74, alpha: tellFade });
    }
    ...
    if (t > T.oneCard && t < T.gather + 0.3) { const a = 1 - E(t, T.slideOut, T.slideOut + 0.5); doodle(ctx, 'do this first →', 700, 400, T.oneCard + 0.5, t, { size: 44, color: P.C.orange, rotate: -0.06, alpha: a }); doodle(ctx, 'why: fresh money · hiring fast', 1290, 700, T.why, t, { size: 38, color: P.C.blue, align: 'center', rotate: 0, dur: 0.9, alpha: a }); serifLine(ctx, 'The one thing to do first.', W / 2, 200, T.oneCard + 0.1, t, { size: 74, alpha: a }); }
    // ── slip: Sweetgreen slides out of the stack, drifts, gets pressed back down by the founder ──
    if (t > T.slideOut && t < T.gather + 1.2) {
      const so = E(t, T.slideOut, T.slideOut + 0.5, ease.inOutCubic); const up = E(t, T.drift, T.drift + 1.6, ease.inOutCubic) * (1 - E(t, T.press, T.press + 0.4, ease.outBounce)); const gth = E(t, T.gather, T.gather + 0.8, ease.inOutCubic);
      const [sx0] = TILE(4); const x0 = lerp(sx0, 1130, so), y0 = GY - tileH; const x = lerp(x0, 1250 - 90, gth), y = lerp(y0 - up * 240, GY - 110 - 12, gth); const alpha = 1 - E(t, T.formed - 0.1, T.formed + 0.1);
      if (up > 0 && up < 1) P.ink(ctx, [[x + tileW / 2, GY - 6], [x + tileW / 2 + 8, GY - 6 - up * 200]], { color: P.C.ink30, width: 2, seed: 1200, boil: b, jitter: 1 });
      if (up > 0.5 && t < T.press) doodle(ctx, 'lifted', x - 110, y - 120, T.drift + 0.9, t, { size: 40, color: P.C.ink70, rotate: -0.08, alpha: 1 - E(t, T.press - 0.2, T.press) });
      ...
      if (st > 0 && t < T.press + 0.2) { const sx = lerp(-700, 1110, st), sy = lerp(GY - 58, y + tileH + 8, held); P.paperLabel(ctx, "smallest step: book Thursday's call", sx, sy, { font: 'hand', size: 32, weight: 700, fill: P.C.bluePaper, seed: 1210, rotate: -0.01, pad: 14, padY: 4 }); }
    ...
    if (t > T.formed) {
      ...
      let cx = bx, cy = lerp(GY - 300, GY - 130, land); let scale = 1; if (pick > 0) { cx = lerp(bx, hireX - 175, pick); cy = lerp(cy, GY - 150, pick); scale = lerp(1, 0.7, pick); }
      ...
      for (let i = 0; i < 7; i++) { const p = E(t, T.tabs + i * 0.09, T.tabs + i * 0.09 + 0.3, ease.outBackSoft); if (p <= 0) continue; const y = -110 + i * 32; P.paperRect(ctx, 150, y, 36 + 130 * p, 26, { fill: i % 2 ? P.C.cream : P.C.paper, seed: 1300 + i, shadow: 0.12, dy: 2, amp: 1 }); P.text(ctx, BOOK_PARTS[i], 162, y + 19, { font: 'hand', size: 19, weight: 700, alpha: p }); }
  ...
  function endCard(ctx, t) {
    ...
    if (t > T.ctaLine) P.ink(ctx, P.partial([[120, GY], [W - 120, GY + 1]], E(t, T.ctaLine, T.ctaLine + 0.8, ease.inOutCubic)), { width: 7, seed: 9, boil: b, jitter: 1.1 });
    const mp = E(t, T.mark, T.mark + 1.2, ease.inOutCubic);
    if (mp > 0) P.mark(ctx, 600, GY, 560, { progress: 0.3 + mp * 0.7, boil: b, groundLen: 0, stroke: 3.2 });
    if (t > T.mark + 1.3) { const k = E(t, T.mark + 1.3, T.mark + 1.6, ease.outBack); P.ink(ctx, [[818, GY - 8], [830, GY - 28 * k]], { color: P.C.forest, width: 5, seed: 1500, boil: b, jitter: 0.5 }); }
    serifLine(ctx, 'Antaeus', 1020, GY - 330, T.name, t, { size: 160, align: 'left' });
    serifLine(ctx, cues.tagline || 'Your first hire could run this.', 1024, GY - 228, T.tagline, t, { size: 56, align: 'left', italic: true, color: P.C.ink70 });
    const up = E(t, T.url, T.url + 0.6, ease.outBackSoft);
    if (up > 0) P.paperLabel(ctx, 'antaeus.app', 1020, GY - 140 + (1 - up) * 120, { font: 'serif', size: 80, fill: P.C.paper, seed: 1510, rotate: -0.01, pad: 22, padY: 6, alpha: up });
    const bp = E(t, T.button, T.button + 0.6, ease.outBackSoft); const breathe = 1 + 0.012 * Math.sin((t - T.button) * 1.6);
    if (bp > 0) { const bx = 1020, by = GY + 34 + (1 - bp) * 200; ctx.save(); ctx.translate(bx + 150, by + 34); ctx.scale(breathe, breathe); ctx.translate(-(bx + 150), -(by + 34)); P.paperRect(ctx, bx, by, 300, 68, { fill: P.C.orange, seed: 1530, shadow: 0.22, dy: 6, outline: 4, texture: 0.08, amp: 1.4 }); P.text(ctx, 'Try the demo', bx + 150, by + 46, { font: 'sans', size: 30, weight: 700, color: '#ffffff', align: 'center' }); ctx.restore();
    ...
    doodle(ctx, 'free · no card needed', 1020, GY + 150, T.ctaNotes, t, { size: 30, color: P.C.ink70, rotate: -0.02 });
    doodle(ctx, 'a sample company inside', 1020, GY + 190, T.ctaNotes + 0.4, t, { size: 30, color: P.C.ink70, rotate: -0.02 });
    doodle(ctx, 'works beside what you already use', 1350, GY + 150, T.ctaNotes + 0.8, t, { size: 30, color: P.C.ink70, rotate: -0.02 });
    // the loud note, asleep on a neat little stack
    if (t > T.zzz - 0.3) { const a = E(t, T.zzz - 0.3, T.zzz, ease.outCubic); ctx.save(); ctx.globalAlpha = a; for (let i = 0; i < 5; i++) P.stickyNote(ctx, 150, GY + 20 - i * 5, 80, { seed: 1540 + i, rotate: 0.02, color: i % 2 ? P.C.sticky2 : P.C.sticky, text: '' }); P.stickyNote(ctx, 146, GY - 12, 90, { seed: 1550, rotate: -0.03, color: P.C.sticky2, text: '' }); P.ink(ctx, [[170, GY + 30], [212, GY + 30]], { width: 3, seed: 1551, boil: b, jitter: 0.5 }); P.text(ctx, 'zzz', 250, GY - 20 - Math.sin(t * 1.4) * 6, { font: 'hand', size: 30 * E(t, T.zzz, T.zzz + 0.4), weight: 700, color: P.C.ink50, rotate: -0.2 }); ctx.restore(); }
    P.grain(ctx, W, H); P.vignette(ctx, W, H, 0.08);
  }
  ...
  const zoomFor = (t) => { // slow, deliberate push-ins per beat; never a cut
    const base = 1.06; const push = (a, bb, k) => k * E(t, a, bb, ease.inOutCubic);
    return base + push(B.hook.start, B.head.end, 0.06) - push(B.myth.start, B.myth.start + 0.9, 0.08) + push(B.tell.start, B.morning.end, 0.03) + push(B.slip.start, B.slip.end, 0.03) - push(B.book.start, B.book.start + 0.8, 0.05);
  };
