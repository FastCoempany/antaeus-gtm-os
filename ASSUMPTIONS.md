...
## Settled by Antaeus (2026-09-15)

- **The AESDR source was fetched, against brief 4.1.** The `aesdr` path in `sources.local.json` did not exist when the build started. Brief 4.1 says not to fetch or clone anything, to note the missing path here, to derive that source's tokens from section 5.2, and to mark every value derived. That instruction was overridden: the repository is public, it was attached read-only to the workspace, and `tokens/aesdr.css` was transcribed from it (about 140 lines marked `from source`, each cited in `SOURCES.md` part B) instead of being derived. The reason was fidelity: the brief pins only AESDR's four faces and asks for its gradient and paper to be derived, and the real product defines an iris, a paper, an ink and a lesson-player contrast that a derivation would have guessed at. Nothing was copied out of it beyond values and anatomy, per rule 1. **Decided: accepted.** `tokens/aesdr.css` keeps its transcribed values and `SOURCES.md` part B keeps its AESDR rows. The override of brief 4.1 stands as recorded here; nothing was rebuilt.
- **The repository stays private; only `dist/` is ever published. Decided, and the README says so.** `SOURCES.md` is the paper trail that makes the token work auditable, and to do that it cites file paths inside four private repositories and names about sixty client asset files. None of it reaches the built pages, which carry no path, employer, partner or product name. Scrubbing those citations was offered and rejected, because the trace from a value back to the file it came from is the point of the document.
